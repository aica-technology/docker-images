...
func main() {
	...
	if schema, ok := instance["schema"].(string); ok {
		schemaURL = "https://raw.githubusercontent.com/aica-technology/api/refs/heads/main/docs/static/schemas/" + schema + "/extension.schema.json"
	} else if schema, ok := instance["$schema"].(string); ok {
		schemaURL = schema
	} else {
		fmt.Println("No 'schema' or '$schema' tag found in the instance.")
		os.Exit(1)
		return
	}
	...
	if err != nil {
		if vErrs, ok := err.(*jsonschema.ValidationError); ok {
			fmt.Println("Instance is not valid against the specified schema. Errors:")
