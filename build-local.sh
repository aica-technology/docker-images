...
cd "${SCRIPT_DIR}"/ros2_control && bash ./build.sh --local-base \
  --base-tag "${BASE_TAG}" --output-tag "${BASE_TAG}" "${BUILD_FLAGS[@]}" || exit 1
...
cd "${SCRIPT_DIR}"/ros2_modulo_control && bash ./build.sh --local-base --modulo-branch "${MODULO_BRANCH}" \
